...
import { getExpensesByMonth } from "../firestoreFns/expense/getExpenseByMonth";
...
const CalendarNav = ({ dateSelected, setDateSelected }: Props) => {
	...
	const [monthsTotalExpense, setMonthsTotalExpense] = useState<number>(0);
	...
	useEffect(() => {
		if (firestoreUserDocId) {
			getExpensesByMonth(dateSelected, firestoreUserDocId).then((res) => {
				// console.log("expense by month", res?.docs);
				const expenses = res?.docs.map((expense) => expense.data().amount);
				if (expenses && expenses.length > 0) {
					const totalExpense = expenses.reduce((total, amount) => {
						return total + amount;
					}, 0);
					setMonthsTotalExpense(totalExpense);
				} else {
					setMonthsTotalExpense(0);
				}
			});
		}
	}, [dateSelected]);
					...
					<div className="stat flex ">
						<div>
							...
							<div className="stat-desc text-base sm:text-lg md:text-xl">
								{useFormatCurrency(monthsTotalExpense)}
							</div>
						</div>
